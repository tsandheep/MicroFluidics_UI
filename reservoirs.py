...
class Right_Reservoir(QWidget):
    def __init__(self,  w, h, row, col, store, color):
        ...
        self.rectangles = [
            {'color': QColor(0, 255, 0), 'rect': (int(button_width/2), 0, button_width, button_height)},
            {'color': QColor(255, 0, 0), 'rect': (0, button_height, button_width, button_height)},
            {'color': QColor(0, 0, 255), 'rect': (button_width+int(button_width/2), 0, button_width, button_height)},
            {'color': QColor(0, 0, 255), 'rect': (button_width+int(button_width/2), 2*button_height, button_width, button_height)},
            {'color': QColor(0, 0, 255), 'rect': (2 * button_width, 0, button_width, 3*button_height)},
            {'color': QColor(0, 255, 0), 'rect': (int(button_width/2), 2*button_height, button_width, button_height)},
            {'color': QColor(0, 0, 0), 'rect': (1*button_width, button_height, button_width, button_height)}
        ]
    ...
    def mousePressEvent(self, event):
        for rectangle in self.rectangles[4:5]:
            rect = rectangle['rect']
            if rect[0] < event.x() < rect[0] + rect[2] and rect[1] < event.y() < rect[1] + rect[3]:
                # Change the color when clicked
                rectangle['color'] = QColor(0, 255, 255)  # Cyan
                self.update()  # Trigger a repaint

        self.store.button_states[self.row][self.col] = 1
        self.store.all_buttons[self.row][self.col].setChecked(True)
        self.store.all_buttons[self.row][self.col].setStyleSheet("""QPushButton { background-color: red }
                            QPushButton:checked {background-color: %s}
                           QPushButton:pressed { background-color: green }"""%(self.color))
    ...
    def mouseReleaseEvent(self, event):
        for rectangle in self.rectangles[4:5]:
            ...
            if rect[0] < event.x() < rect[0] + rect[2] and rect[1] < event.y() < rect[1] + rect[3]:
                # Change the color when the mouse is released
                rectangle['color'] = QColor(0, 0, 255)  # Cyan
                self.update()  # Trigger a repaint
